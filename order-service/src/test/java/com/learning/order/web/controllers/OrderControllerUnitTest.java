...
import static com.learning.order.testdata.TestDataFactory.*;
...
public class OrderControllerUnitTest {
    ...
    @BeforeEach
    void setUp() {
        given(securityService.getLoginUserName()).willReturn("anass");
    }

    @ParameterizedTest(name = "[{index}]-{0}")
    @MethodSource("createOrderRequestProvider")
    void shouldReturnBadRequestWhenOrderPayloadIsInvalid(CreateOrderRequest request) throws Exception {
        given(orderService.createOrder(eq("anass"), any(CreateOrderRequest.class)))
                .willReturn(null);
